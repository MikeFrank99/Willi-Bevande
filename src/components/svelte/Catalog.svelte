<script lang="ts">
  ...
  $: beersForBrand = initialBeers.filter(
    (beer) =>
      (selectedStyles.length === 0 ||
        selectedStyles.includes(beer.data.style)) &&
      (selectedColors.length === 0 ||
        selectedColors.includes(beer.data.color)) &&
      (selectedCountries.length === 0 ||
        selectedCountries.includes(beer.data.country)) &&
      (selectedFormats.length === 0 ||
        beer.data.format.some((f) => selectedFormats.includes(f))) &&
      beer.data.abv <= maxAbv,
  );
  $: beersForStyle = initialBeers.filter(
    (beer) =>
      (selectedBrands.length === 0 ||
        selectedBrands.includes(beer.data.brand)) &&
      (selectedColors.length === 0 ||
        selectedColors.includes(beer.data.color)) &&
      (selectedCountries.length === 0 ||
        selectedCountries.includes(beer.data.country)) &&
  ...
  $: beersForColor = initialBeers.filter(
    (beer) =>
      (selectedBrands.length === 0 ||
        selectedBrands.includes(beer.data.brand)) &&
      (selectedStyles.length === 0 ||
  ...
  $: beersForCountry = initialBeers.filter(
    (beer) =>
      (selectedBrands.length === 0 ||
        selectedBrands.includes(beer.data.brand)) &&
      (selectedStyles.length === 0 ||
      ...
      (selectedAvailabilities.length === 0 ||
        selectedAvailabilities.includes(beer.data.availability)) &&
      beer.data.abv <= maxAbv,
  );
  $: beersForAvailability = initialBeers.filter(
    (beer) =>
      (selectedBrands.length === 0 ||
        selectedBrands.includes(beer.data.brand)) &&
      (selectedStyles.length === 0 ||
  ...
  $: beersForFormat = initialBeers.filter(
    (beer) =>
      (selectedBrands.length === 0 ||
        selectedBrands.includes(beer.data.brand)) &&
      (selectedStyles.length === 0 ||
  ...
  $: brands = (() => {
    const allValues = [
      ...new Set(initialBeers.map((b) => b.data.brand)),
    ].sort();
    return allValues
      .map((val) => ({
        name: val,
        count: beersForBrand.filter((b) => b.data.brand === val).length,
      }))
  ...
  $: styles = (() => {
    const allValues = [
      ...new Set(initialBeers.map((b) => b.data.style)),
    ].sort();
    return allValues
      .map((val) => ({
        name: val,
        count: beersForStyle.filter((b) => b.data.style === val).length,
      }))
  ...
  $: colors = (() => {
    const allValues = [
      ...new Set(initialBeers.map((b) => b.data.color)),
    ].sort();
    return allValues
      .map((val) => ({
        name: val,
        count: beersForColor.filter((b) => b.data.color === val).length,
      }))
  ...
  $: countries = (() => {
    const allValues = [
      ...new Set(initialBeers.map((b) => b.data.country)),
    ].sort();
    return allValues
      .map((val) => ({
        name: val,
        count: beersForCountry.filter((b) => b.data.country === val).length,
      }))
  ...
  $: formats = (() => {
    const allValues = [
      ...new Set(initialBeers.flatMap((b) => b.data.format)),
    ].sort();
    return allValues
      .map((val) => ({
        name: val,
        count: beersForFormat.filter((b) => b.data.format.includes(val)).length,
      }))
  ...
  $: availabilitiesList = (() => {
    ...
    return allValues.map((val) => ({
      ...
      count: beersForAvailability.filter((b) => b.data.availability === val)
        .length,
    }));
  })();

  $: abvs = [...new Set(initialBeers.map((b) => b.data.abv))].sort(
    (a, b) => a - b,
  );
  ...
  $: activeFilterTags = [
    ...selectedBrands.map((val) => ({
      key: `brand-${val}`,
      label: `Marca: ${val}`,
      clear: () => toggleFilter("brand", val),
    })),
    ...selectedStyles.map((val) => ({
      key: `style-${val}`,
      label: `Stile: ${val}`,
      clear: () => toggleFilter("style", val),
    })),
    ...selectedColors.map((val) => ({
      key: `color-${val}`,
      label: `Colore: ${val}`,
      clear: () => toggleFilter("color", val),
    })),
    ...selectedCountries.map((val) => ({
      key: `country-${val}`,
      label: `Nazione: ${val}`,
      clear: () => toggleFilter("country", val),
    })),
    ...selectedFormats.map((val) => ({
      key: `format-${val}`,
      label: `Formato: ${val}`,
      clear: () => toggleFilter("format", val),
    })),
  ...
  onMount(() => {
    ...
    if (params.has("brand"))
      selectedBrands = (params.get("brand") || "")
        .split(",")
        .filter((v) => v && validBrands.has(v));
    if (params.has("style"))
      selectedStyles = (params.get("style") || "")
        .split(",")
        .filter((v) => v && validStyles.has(v));
    if (params.has("color"))
      selectedColors = (params.get("color") || "")
        .split(",")
        .filter((v) => v && validColors.has(v));
    if (params.has("country"))
      selectedCountries = (params.get("country") || "")
        .split(",")
        .filter((v) => v && validCountries.has(v));
    if (params.has("format"))
      selectedFormats = (params.get("format") || "")
        .split(",")
        .filter((v) => v && validFormats.has(v));
    const validAvailabilities = new Set([
      "disponibile",
      "ordinazione",
      "stagionale",
    ]);
    if (params.has("availability"))
      selectedAvailabilities = (params.get("availability") || "")
        .split(",")
        .filter((v) => v && validAvailabilities.has(v));
  ...
  $: {
    if (!isInitializing && typeof window !== "undefined") {
      ...
      if (selectedFormats.length > 0)
        url.searchParams.set("format", selectedFormats.join(","));
      else url.searchParams.delete("format");
      if (selectedAvailabilities.length > 0)
        url.searchParams.set("availability", selectedAvailabilities.join(","));
      else url.searchParams.delete("availability");
      if (maxAbv < actualMaxAbv)
        url.searchParams.set("maxAbv", maxAbv.toString());
  ...
  function toggleFilter(category: string, value: string) {
    if (category === "brand") {
      ...
      selectedStyles = selectedStyles.includes(value)
        ? selectedStyles.filter((s) => s !== value)
        : [...selectedStyles, value];
    } else if (category === "color") {
      selectedColors = selectedColors.includes(value)
        ? selectedColors.filter((c) => c !== value)
        : [...selectedColors, value];
    } else if (category === "country") {
      selectedCountries = selectedCountries.includes(value)
      ...
      selectedAvailabilities = selectedAvailabilities.includes(value)
        ? selectedAvailabilities.filter((a) => a !== value)
...
<div class="catalog-layout">
  <div class="mobile-actions">
    <button class="btn-toggle-filters" on:click={toggleFilters}>
      <svg
        ...
        ></line><line x1="20" y1="21" x2="20" y2="16"></line><line
          ...
          x2="20"
          y2="3"
        ></line><line x1="2" y1="14" x2="6" y2="14"></line><line
          x1="10"
          y1="12"
          x2="14"
          y2="12"
    ...
    <div class="results-info-mobile">
      MOSTRANDO <strong>{filteredBeers.length}</strong>
      {filteredBeers.length === 1 ? "PRODOTTO" : "PRODOTTI"}
    </div>
  </div>
  ...
  <div class="sidebar-column">
    <aside class="filters-panel" class:open={isFiltersOpen}>
      <div class="sidebar-header">
        ...
        <button class="btn-close-filters" on:click={toggleFilters}
          >&times;</button
        >
      </div>
      ...
      <div class="filters-scroll">
        ...
        <div class="filter-group dropdown-group">
          ...
          <div
            ...
            class:is-open={openDropdown === "brand"}
          >
            <button
              class="dropdown-trigger"
              on:click|stopPropagation={() => toggleDropdown("brand")}
            >
              ...
              <div class="dropdown-panel">
                <div class="filter-options-list">
                  {#each brands as b}
                    <label
                      class="checkbox-item"
                      class:selected={selectedBrands.includes(b.name)}
                      class:option-zero={b.count === 0}
                    >
                      <input
                        type="checkbox"
                        checked={selectedBrands.includes(b.name)}
                        on:change={() => toggleFilter("brand", b.name)}
                      />
        ...
        <div class="filter-group dropdown-group">
          ...
          <div
            class="dropdown-wrapper"
            class:is-open={openDropdown === "style"}
          >
            <button
              class="dropdown-trigger"
              on:click|stopPropagation={() => toggleDropdown("style")}
            >
              <span class="trigger-label"
                >{getSelectedLabel(
                  "style",
              ...
              <div class="dropdown-panel">
                <div class="filter-options-list">
                  {#each styles as s}
                    <label
                      class="checkbox-item"
                      class:selected={selectedStyles.includes(s.name)}
                      class:option-zero={s.count === 0}
                    >
                      <input
                        type="checkbox"
                        checked={selectedStyles.includes(s.name)}
                        on:change={() => toggleFilter("style", s.name)}
                      />
                      <span class="checkbox-custom"></span>
        ...
        <div class="filter-group dropdown-group">
          ...
          <div
            class="dropdown-wrapper"
            class:is-open={openDropdown === "color"}
          >
            <button
              class="dropdown-trigger"
              on:click|stopPropagation={() => toggleDropdown("color")}
            >
              <span class="trigger-label"
                >{getSelectedLabel(
                  "color",
                ...
                <div class="filter-options-list">
                  {#each colors as c}
                    <label
                      ...
                      class:option-zero={c.count === 0}
                    >
                      <input
                        type="checkbox"
                        checked={selectedColors.includes(c.name)}
                        on:change={() => toggleFilter("color", c.name)}
                      />
                      <span class="checkbox-custom"></span>
        ...
        <div class="filter-group dropdown-group">
          ...
          <div
            class="dropdown-wrapper"
            class:is-open={openDropdown === "availability"}
          >
            <button
              class="dropdown-trigger"
              on:click|stopPropagation={() => toggleDropdown("availability")}
            >
              <span class="trigger-label"
                >{getSelectedLabel(
                  "availability",
              ...
              <div class="dropdown-panel">
                <div class="filter-options-list">
                  {#each availabilitiesList as a}
                    <label
                      class="checkbox-item availability-{a.id}"
                      class:selected={selectedAvailabilities.includes(a.id)}
                      class:option-zero={a.count === 0}
                    >
                      <input
                        type="checkbox"
                        checked={selectedAvailabilities.includes(a.id)}
                        on:change={() => toggleFilter("availability", a.id)}
                      />
                      <span class="checkbox-custom"></span>
        ...
        <div class="filter-group dropdown-group">
          ...
          <div
            class="dropdown-wrapper"
            class:is-open={openDropdown === "country"}
          >
            <button
              class="dropdown-trigger"
              on:click|stopPropagation={() => toggleDropdown("country")}
            >
              <span class="trigger-label"
                >{getSelectedLabel(
                  "country",
                  ...
                  {#each countries as c}
                    ...
                    >
                      <input
                        ...
                        on:change={() => toggleFilter("country", c.name)}
                      />
                      <span class="checkbox-custom"></span>
                      <span class="item-name">{c.name}</span>
                      <span class="item-count">({c.count})</span>
        ...
        <div class="filter-group dropdown-group">
          ...
          <div
            class="dropdown-wrapper"
            class:is-open={openDropdown === "format"}
          >
            <button
              class="dropdown-trigger"
              on:click|stopPropagation={() => toggleDropdown("format")}
            >
              <span class="trigger-label"
                >{getSelectedLabel(
                  "format",
              ...
              <div class="dropdown-panel">
                <div class="filter-options-list">
                  {#each formats as f}
                    <label
                      class="checkbox-item"
                      class:selected={selectedFormats.includes(f.name)}
                      class:option-zero={f.count === 0}
                    >
                      <input
                        type="checkbox"
                        checked={selectedFormats.includes(f.name)}
                        on:change={() => toggleFilter("format", f.name)}
                      />
                      <span class="checkbox-custom"></span>
          ...
          <div class="range-container">
            <input
              ...
              on:change={(e) => {
                ...
                const closest = abvs.reduce((prev, curr) =>
            ...
            <div class="custom-ticks">
              {#each abvs as abv}
                <div
                  class="tick"
                  style="left: {((abv - actualMinAbv) /
                    (actualMaxAbv - actualMinAbv)) *
                    100}%"
                ></div>
              {/each}
            </div>
    ...
    {#if showScrollToFilters && !isFiltersOpen}
      ...
      >
        <button
          class="btn-toggle-filters floating-mobile-clone"
          on:click={toggleFilters}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
  ...
  <main class="products-grid">
      ...
      {#if hasActiveFilters}
        ...
        >
          {#each activeFilterTags as tag (tag.key)}
            <button
              class="filter-tag {tag.extraClass || ''}"
              on:click={tag.clear}
    ...
    {#if filteredBeers.length === 0}
      <div class="no-results">
        Nessuna birra corrisponde ai criteri selezionati. <button
          on:click={resetFilters}>Prova a svuotare i filtri</button
        >.
      </div>
    {:else}
      <div class="grid">
        {#each filteredBeers as beer (beer.id)}
          <a href="{base}catalogo/{beer.slug}/" class="beer-card">
            <div
              class="card-logo-bg"
              style="background-image: url({base}images/logos/Willi_Logo_Beer.svg)"
            ></div>
            ...
            <div class="card-info">
              ...
              <div class="specs-list">
                <div class="spec-group">
                  <span class="label">Stile</span><span class="value"
                    >{beer.data.style}</span
                  >
                </div>
                <div class="spec-group">
                  <span class="label">Colore</span><span class="value"
                    >{beer.data.color}</span
                  >
                </div>
                <div class="spec-group">
              ...
              <div class="availability-dot-container {beer.data.availability}">
                ...
                <span class="status-text">
                  {beer.data.availability === "disponibile"
                    ? "DISPONIBILE ORA"
                    : beer.data.availability === "ordinazione"
                      ? "DA ORDINARE"
...
<style lang="scss">
  ...
  .filter-group {
    ...
    .checkbox-item {
      ...
      &.availability-disponibile .item-name {
        color: #2e7d32;
        font-weight: 600;
      }
      &.availability-ordinazione .item-name {
        color: #f57f17;
        font-weight: 600;
      }
      &.availability-stagionale .item-name {
        color: #c62828;
        font-weight: 600;
      }
  ...
  .beer-card {
    ...
    background: #ffffff;
    border: 2px solid;
    border-image: linear-gradient(to bottom, var(--color-primary), #000000) 1;
    transition: all 0.5s cubic-bezier(0.2, 1, 0.3, 1);
    position: relative;
    height: 100%;
    border-radius: 0;
    overflow: hidden;
    box-shadow: 0 4px 12px rgba(255, 77, 69, 0.08);
    ...
    .img-wrapper {
      ...
      border: none;
      border-bottom: 2px solid #a6322d;
      position: relative;
      transition: all 0.5s ease;
      overflow: hidden;
      z-index: 1;
    ...
    &:hover {
      background: #fdfdfd;
      border-image: linear-gradient(to bottom, #ff6b62, var(--color-primary)) 1;
      box-shadow: 0 10px 25px rgba(255, 77, 69, 0.18);
      &:before {
        transform: scaleX(1);
      }
      .card-logo-bg {
